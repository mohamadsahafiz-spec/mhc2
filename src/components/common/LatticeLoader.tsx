...
export interface LatticePatternObject {
  cells: (number | null)[];
  loop?: number;
  scale?: number;
  lit?: number;
...
export type LatticePattern = string | LatticePatternObject;
...
const PATTERNS: Record<string, Record<number, LatticePatternObject>> = {
...
const resolvePattern = (pattern: LatticePattern, grid: number): LatticePatternObject => {
  ...
  const max = numericCells.length > 0 ? Math.max(...numericCells) : 0;
