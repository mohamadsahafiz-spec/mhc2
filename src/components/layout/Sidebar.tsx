...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <motion.aside
      ...
      className="w-60 border-r flex flex-col h-screen sticky top-0 shrink-0 select-none z-30 overflow-hidden backdrop-theme-surface bg-surface border-theme-default text-theme-primary"
    >
      <div className="w-60 flex flex-col h-full shrink-0">
        {/* 1. Header Branding & Collapse Action */}
        <div className="h-14 px-3.5 border-b border-theme-default flex items-center justify-between shrink-0">
        ...
        <div className="px-3 py-2 border-b border-theme-default">
          <motion.button 
            ...
            className={`w-full flex items-center justify-between px-2.5 py-1.5 rounded-button text-left transition-colors relative ${
              activeTab === 'profile'
                ? 'bg-raised text-theme-primary border border-theme-strong shadow-2xs' 
        ...
        <nav className="flex-1 overflow-y-auto p-2.5 space-y-4 scrollbar-thin">
          {navGroups.map((group) => (
            <div key={group.key} className="space-y-1">
              {/* Section Header */}
              <div className="px-2 py-0.5 text-[10px] font-mono font-medium tracking-wider uppercase text-theme-muted">
                {group.title}
              </div>
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={`w-full flex items-center justify-between px-2.5 py-1.5 rounded-button text-xs transition-colors relative ${
                        isActive
                          ? 'bg-raised text-theme-primary font-medium border border-theme-strong shadow-2xs'
        ...
        <div className="px-3.5 py-2.5 border-t border-theme-default text-[10px] font-mono flex items-center justify-between shrink-0 bg-surface text-theme-muted">
